...
class BipartiteSwapNetworkGate(PermutationGate):
    ...
    def permutation(self) -> Dict[int, int]:
        ...
        if self.subgraph == BipartiteGraphType.MATCHING:
            return dict(
                enumerate(
                    itertools.chain(*(range(self.part_size + offset -
                                            1, offset - 1, -1)
    ...
    def _circuit_diagram_info_(self, args: protocols.CircuitDiagramInfoArgs
        ...
        permutation = {
            i: partial_permutation.get(i, i) for i in range(qubit_count)
        }
        ...
        wire_symbols = tuple(
            name + ':' + str((i // self.part_size, i % self.part_size)) +
            arrow + str((j // self.part_size, j % self.part_size))
