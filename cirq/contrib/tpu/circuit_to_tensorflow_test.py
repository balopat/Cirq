...
from cirq.contrib.tpu import (circuit_to_tensorflow_runnable)
import cirq
...
def test_circuit_to_compute_and_feed_dict_vector_custom_start_state():
    ...
    with pytest.raises(ValueError):
        _ = circuit_to_tensorflow_runnable(c, initial_state='zero please')
...
def test_circuit_to_compute_and_feed_dict_works_on_unknown_ops():
    ...
    c = cirq.Circuit.from_ops(
        ...
        cirq.CCX(qs[0], qs[4], qs[8])**0.5, phased_swap(qs[0], qs[1]),
        phased_swap(qs[3], qs[9]), phased_swap(qs[0], qs[6]),
