...
def test_init_from_double_map_vs_kwargs(trans1, trans2, from1):
    ...
    gate_map = cirq.SingleQubitCliffordGate.from_double_map({
        from1: trans1,
        from2: trans2
    })
...
def test_init_from_double(trans1, trans2, from1):
    from2 = cirq.Pauli.by_relative_index(from1, 1)
    gate = cirq.SingleQubitCliffordGate.from_double_map({
        from1: trans1,
        from2: trans2
    })
...
def test_eq_ne_and_hash():
    eq = EqualsTester()
    for trans_x, trans_z in _all_rotation_pairs():
        gate_gen = lambda: cirq.SingleQubitCliffordGate.from_xz_map(
            trans_x, trans_z)
...
def test_inverse_matrix(gate):
    ...
    assert_allclose_up_to_global_phase(mat,
                                       mat_inv.T.conj(),
                                       rtol=1e-7,
                                       atol=1e-7)
