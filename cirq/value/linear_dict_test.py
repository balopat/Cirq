...
@pytest.mark.parametrize('terms, expected_length', (({}, 0), ({
    'X': 0
}, 0), ({
    'X': 0.1
}, 1), ({
...
@pytest.mark.parametrize('expression, expected', (
    ((cirq.LinearDict({'X': 10}) + cirq.LinearDict({
        'X': 10,
        'Y': -40
    })) / 20, cirq.LinearDict({
        'X': 1,
...
@pytest.mark.parametrize('terms, fmt, expected_string', (
    ...
    ({
        ...
        'Y': -1,
        'Z': 1j
    }, '{:.5f}', '1.00000*X-1.00000*Y+1.00000j*Z'),
    ({
        'X': 2,
        'Y': -0.0001
    }, '{:.4f}', '2.0000*X-0.0001*Y'),
    ({
        'X': 2,
        'Y': -0.0001
    }, '{:.3f}', '2.000*X'),
