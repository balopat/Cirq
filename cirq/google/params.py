...
def sweep_to_proto_dict(sweep: Sweep, repetitions: int = 1) -> Dict:
    ...
    if not sweep == UnitSweep:
        ...
        msg['sweep'] = {
            'factors': [
                _sweep_zip_to_proto_dict(cast(Zip, factor))
...
def _sweep_zip_to_proto_dict(sweep: Zip) -> Dict:
    sweeps = [
        _single_param_sweep_to_proto_dict(cast(SingleSweep, s))
        for s in sweep.sweeps
    ]
    return {'sweeps': sweeps}


def _single_param_sweep_to_proto_dict(sweep: SingleSweep,) -> Dict:
    msg = {}  # type: Dict
